...
/**
 * @ExtendWith(MockitoExtension.class) - JUnit5 테스트에서 Mockito 프레임워크를 통한 목(Mock)
 * 객체를 사용하여 테스트 할 수 있도록 해주는 기능. 요약하면 Mockito 프레임워크를 사용한 확장 테스트 기능을 부여함.
...
class EmployeeControllerTest {
	...
	/*
	 * @InjectMocks : 가상으로 만들어질 목 객체를 이용해서 테스트를 진행할 목 객체(컨트롤러) 의존성 주입
	 */
	@InjectMocks
	...
	public void testGetEmployeeList() {
		...
		/*
		 * [1] when().then() 으로 결과를 가정 - when : 가상으로 서비스 레이어의 사원 목록 조회 메소드 호출했을 때 
		 * - thenReturn(employeesList) : 위에서 만든 ArrayList가 되돌아오도록 설정. 진행 시나리오를 설정.
		 */
		EmployeeCommonDto dto3 = new EmployeeCommonDto(); // 형식적으로 전달
		when(employeeService.getEmployeesList(dto3)).thenReturn(employeesList);

		/*
		 * [2] 직접 컨트롤러 메소드 호출(실제 동작 실행) 컨트롤러의 getEmployeeList 메소드를 호출하게 되면 
		 ...
		 *  - model이라는 Mock 객체에 list라는 이름으로 employeesList가 저장되어 있는지 검증. 
		 *  - list이름은 스펠링 틀리면 안됨 
	...
	public void testGetEmployee() {
		...
		/*
		 * [1] when.then으로 결과를 가정
		 *  - when : 가상으로 서비스 레이어의 사원 목록 조회 메소드 호출했을 때
		 *  - thenReturn(dto) : 위에서 만든 dto가 되돌아오도록 설정. 시나리오 짜놓음. 
		 */
		when(employeeService.getEmployees(employeeId)).thenReturn(dto);
		 ...
		 * 
		 * Mockito의 verify() 메소드로 1차 검증
		 *  - model이라는 Mock 객체에 employee 라는 이름으로 employee가 저장되어 있는지 검증.
	...
	public void testRegister() {
		...
		// [2] employeeService.register() 메소드의 동작 모의(시나리오) 
		// register 메소드를 호출하면서 위에서 만든
		// emp를 파라미터로 보내면 1을 반환하기로 모의함
		 
		when(employeeService.register(emp)).thenReturn(1);
		
		// [3] 컨트롤러 사원 정보 조회 메소드 호출
		String viewName = employeeController.register(emp);
		
		// [4] 검증, 단정
		// employeeService.register() 메소드가 올바른 직원 개체로 호출되었는지 확인합니다.
