...
public class ProductRepositoryTest {
	 ...
	 * 
	 * // 상품 등록시 사용할 실제로 존재하는 카테고리 객체 조회 Integer categoryId = 1; Category category =
	 * categoryRepository.findById(categoryId).orElseThrow(); // Retrieve an
	 * existing Category entity from the database
	 * 
	 * IntStream.rangeClosed(1, 100).forEach(i -> { Product product = new Product();
	 * product.setProductName("Product Name" + i); product.setPrice(120000);
	 * product.setCategory(category); product.setDescription("상품 설명" + i);
	 * product.setRegDate(LocalDateTime.now());
	 * 
	 * // 영속화(메모리의 엔티티 매니저의 관리 대상이 됨) productRepository.save(product); });
	 * 
	...
	public void testProductSave() {
		...
		Category category = categoryRepository.findById(categoryId).orElseThrow(); // Retrieve an existing Category
																					// entity from the database

		IntStream.rangeClosed(1, 200).forEach(i -> {
			Product product = Product.builder().productName("Product Name" + i).price(120000).category(category)
					.description("상품 설명" + i).regDate(LocalDateTime.now()).build();
	...
	public void testGetProductById2() {
		Optional<Product> optProduct = productRepository.findById(76); // 데이터베이스 확인
	 ...
	 *  - Fetch 전략을 안 주거나, Eager로 하게 되면 처음부터 상품 테이블과 
	 *    카테고리 테이블을 조인걸게 된다. 그렇게 되면 데이터베이스 성능에 좋지 않다.
	...
	public void testGetProductById() {
		Optional<Product> optProduct = productRepository.findById(76); // 데이터베이스 확인
		Product product = optProduct.get();
		log.info("Product 한개 조회 완료 : " + product.getProductName());
		
		// 1. 상품 엔티티의 Category 멤버 변수에 지연로딩을 설정해놓게 되면 
		//    이 시점까지는 상품 테이블만 조회함.
		// 2. 아래의 product.toString()으로 상품의 모든 내용(Category포함)을 
	...
	public void testProductUpdate() {
		...
		product.setProductId(2);	// 실존하는 상품명으로
